...
import { getBlankenburg, getBornholmerStr, getBuch, getGreifswalderStr, getKarow, getPankow, getPankowHeinersdorf, getPrenzlauerAllee, getSchönhauserAllee, getWollankstr } from "./Pankow"
...
export function changeStopObject (mode: string, oldStopObject: Data) {
  ...
  if (['tram', 'bus', 'ferry'].includes(product)) {
    switch (id) {
      ...
      case "900000092201":
      case "900000092282":
        [newStopName, trackNo] = getFrohnau(id, mode, lineName, direction, provenance)
        newStop = { ...stop, name: newStopName }
        return { ...oldStopObject, stop: newStop, platform: trackNo }
      case "900000007102":
      ...
      case "900000180001":
      case "900000180701":
        [newStopName, trackNo] = getKöpenick(id, mode, lineName, direction, provenance)
        newStop = { ...stop, name: newStopName }
        return { ...oldStopObject, stop: newStop, platform: trackNo }
      case "900000023203":
      ...
      case "900000007104":
      case "900000007108":
        [newStopName, trackNo] = getNordbahnhof(id, mode, lineName, direction, provenance)
        newStop = { ...stop, name: newStopName }
        return { ...oldStopObject, stop: newStop, platform: trackNo }
      case "900000100007":
