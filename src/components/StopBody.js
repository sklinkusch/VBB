...
import { getAdenauerplatz, getBismarckstr, getBundesplatz, getCharlottenburg, getErnstReuterPlatz, getFehrbellinerPlatz, getGrunewald, getHalemweg, getHalensee, getHeerstr, getHeidelbergerPlatz, getHohenzollerndamm, getHohenzollernplatz, getJakobKaiserPlatz, getJungfernheide, getKurfürstendamm, getMesseNord, getMesseSüd, getMierendorffplatz, getNeuWestend, getRichardWagnerPlatz, getRuhleben, getRüdesheimerPlatz, getSophieCharlottePlatz, getSpichernstr, getTheo, getUhlandstr, getWestend, getWestkreuz, getZooBusStops } from "./stopHelpers/Charlottenburg-Wilmersdorf"
  ...
  const splitArray = async (data) => {
    if (data !== null && data !== undefined && data.length > 0) {
      const dataModified = await data.map(e => {
        ...
        if(["900000020204", "900000026207"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getWestend(id)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000026204", "900000024106", "900000026202"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getMesseNord(id, lineName)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000024102"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getWestkreuz()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000040101"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getHalensee()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000044101"].includes(id) && ['bus'].includes(product)) {
        ...
        if(["900000044202"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getBundesplatz()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000025423"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getMesseSüd()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000026105"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getHeerstr()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000026101"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getNeuWestend()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000026201"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getTheo()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000022101"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getSophieCharlottePlatz(mode, lineName, direction, provenance)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000024201"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getBismarckstr()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000023101"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getErnstReuterPlatz()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000023203"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getKurfürstendamm(mode, lineName, direction, provenance)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000023301"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getUhlandstr()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        ...
        if(["900000041101"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getFehrbellinerPlatz(lineName)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000045101"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getRüdesheimerPlatz()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000018102"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getHalemweg()
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000018101"].includes(id) && ["bus"].includes(product)) {
          const newStopName = getJakobKaiserPlatz(mode, lineName, direction, provenance)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000019204"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getMierendorffplatz(mode, lineName, direction, provenance)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000022202"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getRichardWagnerPlatz(lineName)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
        if(["900000023302"].includes(id) && ['bus'].includes(product)) {
          const newStopName = getAdenauerplatz(mode, lineName, direction, provenance)
          const newStop = { ...stop, name: newStopName }
          return { ...e, stop: newStop }
        }
